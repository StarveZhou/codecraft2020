...
def compare(std, res):
    ...
    for i in range(len(std_data)):
        ...
        if len(std_line) != len(res_line):
            error_occur(
                "line: {}, diff number of items, A: {}, B: {}".format(i + 1, len(std_line), len(res_line)))
        for j in range(len(std_line)):
            if std_line[j] != res_line[j]:
                error_occur("line: {}, cow: {}, A: {}, B: {}".format(i + 1, j + 1, std_line[j], res_line[j]))
    print("success!")
...
if __name__ == '__main__':
    ...
    if len(args) >= 3:
        result_path = args[2]
    compare(output_path, result_path)
