...
void no_dfs_forw_mt_0() {
    int u_mt_0[8], v_mt_0[8], i_mt_0[8], header_mt_0[8];
    int u, v;
    bool nxt_flag = true;
    while (dfs_path_num_mt_0 > 0) {
        if (nxt_flag) {
            u = u_mt_0[dfs_path_num_mt_0] = dfs_path_mt_0[dfs_path_num_mt_0 - 1];
            ...
            i_mt_0[dfs_path_num_mt_0] = edge_topo_starter_mt_0[u];
        } else {
            u = u_mt_0[dfs_path_num_mt_0];
        }
        nxt_flag = false;
        if (i_mt_0[dfs_path_num_mt_0] >= edge_topo_header[u+1]) {
            visit_mt_0[u] = -1;
            dfs_path_num_mt_0 --;
            continue;
        }
        v = edge_topo_edges[i_mt_0[dfs_path_num_mt_0] ++];
        if (visit_mt_0[v] == mask_mt_0) continue;
