...
struct Answer {
    ...
    Answer() {}
    Answer(int s, int a, int b, int c = INVALID_NODE, int d = INVALID_NODE, int e = INVALID_NODE, int f = INVALID_NODE):
...
inline void calc_answer_size(Answer& answer) {
    int size = 0;
    size = integer_buffer_size[answer.s] + integer_buffer_size[answer.a]
        + integer_buffer_size[answer.b] + 3;
    if (answer.c != INVALID_NODE) {
        size += integer_buffer_size[answer.c] + 1;
        if (answer.d != INVALID_NODE) {
            size += integer_buffer_size[answer.d] + 1;
            if(answer.e != INVALID_NODE) {
                size += integer_buffer_size[answer.e] + 1;
                if (answer.f != INVALID_NODE) {
                    size += integer_buffer_size[answer.f] + 1;
                }
            }
        }
    }
