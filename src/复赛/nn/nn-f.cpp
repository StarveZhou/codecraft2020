...
// #define INPUT_PATH     "resources/test_topo.txt"
...
bool useless_edges[MAX_EDGE];
int* extreme[4];

void do_filter() {
    for (int i=0; i<4; ++i) {
        extreme[i] = (int*) malloc(sizeof(int) * node_num);
        memset(extreme[i], 0, sizeof(int) * node_num);
    }
    int u, v, x;
    for (int i=0; i<data_num; ++i) {
        u = data[i][0];
        ...
        if (extreme[0][v] == 0) extreme[0][v] = x;
        else extreme[0][v] = std::min(extreme[0][v], x);
        extreme[1][v] = std::max(extreme[1][v], x);

        if (extreme[2][u] == 0) extreme[2][u] = x;
        else extreme[2][u] = std::min(extreme[2][u], x);
        extreme[3][u] = std::max(extreme[3][u], x);
    }
    ...
    for (int i=0; i<data_num; ++i) {
        ...
        if (lx*5 < (long long) extreme[0][u] || lx > (long long) extreme[1][u] * 3) {
            useless_edges[i] = true;
        } else if (lx > (long long) extreme[3][v] * 5 || lx * 3 < (long long) extreme[2][v]) {
            useless_edges[i] = true;
        }
    }
    ...
    int counter = 0;
    for (int i=0; i<data_num; ++i) {
        if (useless_edges[i]) counter ++;
    }
    printf("useless edge: %d\n", counter);

    for (int i=0; i<4; ++i) free(extreme[i]);
}
...
void rehash_nodes() {
    ...
    int index;
    for (int i=0; i<edge_topo_header[node_num]; ++i) {
    ...
    printf("edge filter: %d %d %d %d\n", data_num, edge_topo_header[node_num], rev_edge_topo_header[node_num], counter);
...
bool* CACHE_LINE_ALIGN searchable_nodes;
bool* isolated_nodes;
void filter_searchable_nodes() {
    int* malloc_all = (int*) malloc(sizeof(int) * (node_num + node_num));
    memset(malloc_all, 0, sizeof(int) * (node_num + node_num));
    int* flag_from = malloc_all, *flag_to = malloc_all + node_num;

    searchable_nodes = (bool*) malloc(sizeof(bool) * node_num);
    ...
    for (u=0; u<node_num; ++u) {
        for (int j=edge_topo_header[u]; j<edge_topo_header[u+1]; ++j) {
            v = edge_topo_edges[j];
            if (v < u) flag_to[v] = 1;
            if (v > u) flag_from[u] = 1;
        }
    }
    for (u=0; u<node_num; ++u) {
        searchable_nodes[u] = (flag_from[u] > 0) && (flag_to[u] > 0);
    }

    isolated_nodes = (bool*) malloc(sizeof(bool) * node_num);
    memset(isolated_nodes, 0, sizeof(bool) * node_num);

    int* counter = malloc_all;
    int* queue = malloc_all + node_num;
    int head, tail;

    head = tail = 0;
    for (u=0; u<node_num; ++u) {
        counter[u] = rev_edge_topo_header[u+1] - rev_edge_topo_header[u]; 
        if (counter[u] == 0) {
            isolated_nodes[u] = true;
            queue[tail ++] = u;
        }
    }
    while (head < tail) {
    ...
    head = tail = 0;
    for (u=0; u<node_num; ++u) {
        counter[u] = edge_topo_header[u+1] - edge_topo_header[u];
        if (counter[u] == 0) {
            isolated_nodes[u] = true;
            queue[tail ++] = u;
        }
    ...
    // printf("starter?\n");
    // for (int i=0; i<node_num; ++i) {
    //     printf("[%d]: %d\n", data_rev_mapping[i], isolated_nodes[i]);
    // }
    // printf("\n");
...
void* search_mt_0(void* args) {
    ...
    while (true) {
        ...
        if (u >= node_num) break;
        global_assign[u] = -1;
        if ((!searchable_nodes[u]) || isolated_nodes[u]) continue;
        global_assign[u] = 0;
        for (int i=0; i<5; ++i) {
