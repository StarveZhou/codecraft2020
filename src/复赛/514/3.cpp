...
void read_input() {
    ...
    HashPair* hash = (HashPair*) malloc(sizeof(HashPair) * data_num * 2);
    for (int i=0; i<data_num; ++i) {
        hash[i << 1] = {data[i][0], i << 1};
        hash[(i << 1) | 1] = {data[i][1], (i << 1) | 1};
    }
    std::sort(hash, hash + data_num + data_num, [](HashPair a, HashPair b) {
        return a.x < b.x;
    });

    int counter = 0, first, second;
    data_rev_mapping[0] = hash[0].x;
    for (int i=0; i<data_num + data_num; ++i) {
        if (i != 0 && hash[i].x != hash[i-1].x) {
            counter ++;
            data_rev_mapping[counter] = hash[i].x;
        }
        first = hash[i].id >> 1;
        second = hash[i].id & 1;
        data[first][second] = counter;
    }
...
void* search_##tid(void* args) { \
    ...
    while (true) { \
        ...
        global_assign[u] = tid; \
        do_search_##tid(u); \
        __sync_fetch_and_add(&useful_counter, 1); \
    } \
    for (int i=0; i<5; ++i) { \
