        ...
        while (a_num --) { \
            ...
            while (b_num --) { \
                ...
                while (c_num --) { \
                    ...
                    if (topo_useless[d]) continue; \
                    if (d == a || d == b) continue; \
                    if (not_check_x_y(bx, cx)) continue; \
                    \
...
void memcpy_to_answer_all(int aid, int from, int to, int& now) {
    // printf("memcpy into: %d %d %d %d\n", aid, from, to, now);

#define def_memcpy_answer_case(tid) \
        case tid: \
            head = answer_bound_##tid[aid][u].head; \
            tail = answer_bound_##tid[aid][u].tail; \
            memcpy(answer_all + now, answer_##tid[aid] + head, (tail - head)* sizeof(Answer)); \
            now += tail - head; \
            break;

    int u, head, tail;
    for (u=from; u<to; ++u) {
        // printf("memcpy in: %d %d %d %d %d\n", aid, from, to, now, u);
        if (global_assign[u] != -1) {
            switch (global_assign[u])
            {
            def_memcpy_answer_case(0)
            def_memcpy_answer_case(1)
            def_memcpy_answer_case(2)
            def_memcpy_answer_case(3)        
            default:
                break;
            }
        }
        // printf("memcpy out: %d %d %d %d %d\n", aid, from, to, now, u);
    }

    // printf("memcpy out: %d %d %d %d\n", aid, from, to, now);
}

int merge_answer_from[5][3];
int global_merge_counter = 0;

void* merge_thr(void* args) {
    int id = __sync_fetch_and_add(&global_merge_counter, 1);
    int head, tail, now, u;
    now = merge_answer_from[id][2];
    for (int aid=merge_answer_from[id][0]; aid <= merge_answer_from[id+1][0]; ++aid) {
        head = aid == merge_answer_from[id][0] ? merge_answer_from[id][1] : 0;
        tail = aid == merge_answer_from[id+1][0] ? merge_answer_from[id+1][1] : node_num;
        // printf("merge for: %d %d %d %d %d %d\n", id, aid, head, tail, now, tail - head);
        memcpy_to_answer_all(aid, head, tail, now);
        // printf("merge out: %d %d %d %d %d\n", id, aid, head, tail, now);
    }
    printf("merge thr: %d %d\n", id, now);
    return NULL;
}
...
int merge_answer() {
    int now = 0, u, head, tail, total_size = 0, temp_size, div_size, t_s, index;
    Answer* local_answer;

    answer_all = (Answer*) malloc(sizeof(Answer) * total_answer_num);

    printf("total_answer_num: %d\n", total_answer_num);

    t_s = 0; div_size = total_answer_num / 4; index = 0;
...
#define def_merge_answer_for(aid) \
    for (u=0; u<node_num; ++ u) { \
        ...
        if (t_s >= div_size) { \
            t_s = 0; \
            index ++; \
            merge_answer_from[index][0] = aid; \
            merge_answer_from[index][1] = u + 1; \
            merge_answer_from[index][2] = now; \
        } \
    }
    ...
    merge_answer_from[4][0] = 4;
    merge_answer_from[4][1] = node_num;

    // for (int i=0; i<5; ++i) {
    //     printf("merge answer from: %d %d %d\n", merge_answer_from[i][0], merge_answer_from[i][1], merge_answer_from[i][2]);
    // }

    pthread_t merge_t[4];
    for (int i=0; i<4; ++i) {
        pthread_create(merge_t + i, NULL, merge_thr, NULL);
    }
    for (int i=0; i<4; ++i) pthread_join(merge_t[i], NULL);
...
void do_write_to_disk(int id) {
    ...
    for (u=process_answer_from[id]; u<process_answer_from[id+1]-10; ++u) {
        deserialize_answer(ans, now, answer_all[u]);
    }
    for (u=process_answer_from[id+1]-10; u<process_answer_from[id+1]; ++u) {
        deserialize_answer_slow(ans, now, answer_all[u]);
    }
...
void create_integer_buffer() {
    ...
    for (; i<node_num; ++i) {
        ...
        for (l=1,r=integer_buffer_size[i]-1; l<r; ++l, --r) {
