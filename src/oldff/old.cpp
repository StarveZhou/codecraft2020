...
#define PATH_PAR_3             8000000
#define PATH_PAR_4             15000000
...
int*  edge_v = (int*) malloc(MAX_EDGE * sizeof(int));
int*  edge_ptr = (int*) malloc(MAX_EDGE * sizeof(int));
int* edge_header = (int*) malloc(MAX_EDGE * sizeof(int));
int*  edge_value = (int*) malloc(MAX_EDGE * sizeof(int));


int* edge_sorted_id = (int*) malloc(MAX_EDGE * sizeof(int));
...
void rehash_nodes() {
    ...
    // }
    // printf("\n");
    // for (int i=0; i<5; ++i) printf("data: %d %d %d\n", data[i][0], data[i][1], data[i][2]);

    edge_num = 0;
    int sorted_num, from;
    edge_topo_header[0] = 0;
    for (u=0; u<node_num; ++u) {
        sorted_num = 0;
        for (int i=edge_header[u]; i!=0; i=edge_ptr[i]) {
            edge_sorted_id[sorted_num ++] = i;
            edge_num ++;
        }
        edge_topo_header[u+1] = edge_num;
        std::sort(edge_sorted_id, edge_sorted_id + sorted_num, [](int i, int j) {
            return edge_v[i] < edge_v[j];
        });
        for (int i=0; i<sorted_num; ++i) {
            edge_topo_edges[i + edge_topo_header[u]] = edge_v[edge_sorted_id[i]];
            edge_topo_values[i + edge_topo_header[u]] = edge_value[edge_sorted_id[i]];
        }
    }

    // for (int i=0; i<=node_num; ++i) {
    //     printf("%d ", edge_topo_header[i]);
    // }
    // printf("\n");
    ...
    //         printf("%d,%d ", data_rev_mapping[edge_topo_edges[j]], edge_topo_values[j]);
    //     }
    //     printf("\n");
    // }
    ...
    for (u=0; u<node_num; ++u) {
        for (int i=edge_topo_header[u]; i<edge_topo_header[u+1]; ++i) {
            ...
            edge_value[edge_num] = edge_topo_values[i];
            edge_header[v] = edge_num ++;            
        }
    }
    edge_num = 0;
    rev_edge_topo_header[0] = 0;
    for (u=0; u<node_num; ++u) {
        sorted_num = 0;
        for (int i=edge_header[u]; i!=0; i=edge_ptr[i]) {
            edge_sorted_id[sorted_num ++] = i;
        ...
        for (int i=0; i<sorted_num; ++i) {
            rev_edge_topo_edges[i + rev_edge_topo_header[u]] = edge_v[edge_sorted_id[i]];
            rev_edge_topo_values[i + rev_edge_topo_header[u]] = edge_value[edge_sorted_id[i]];
        }
    }
    ...
    // for (int i=0; i<node_num; ++i) {
    //     printf("[%d]: ", data_rev_mapping[i]);
    //     for (int j=rev_edge_topo_header[i]; j<rev_edge_topo_header[i+1]; ++j) {
    //         printf("%d,%d ", data_rev_mapping[rev_edge_topo_edges[j]], rev_edge_topo_values[j]);
    //     }
    //     printf("\n");
    // }
