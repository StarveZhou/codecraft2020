...
int adv_visit[8][MAX_NODE], adv_mask[8];

int total_dfs, total_cut;

bool do_search() {
    total_dfs ++;
    int u, v, mid; 
    bool dfs_flag = false, ret_flag, ext_dfs, need_pass;
    u = dfs_path[depth-1];
    adv_mask[depth-1] ++;
    for (int i=decide_search_start(u); i<edge_topo_header[u+1]; ++i) {
        ...
        if (need_pass) continue;

        if (likely(depth + MAX_SP_DIST >= 7)) {
            if (unlikely(shortest_path_now != dfs_path[0])) shortest_path(dfs_path[0]);
            if (likely(sp_dist[v] == -1 || sp_dist[v] + depth > 7)) {
                adv_visit[depth-1][v] = adv_mask[depth-1];
                continue;
            }
        }
