...
Edge *__fwd_edges, *__bck_edges, *__fwd_edge_edges;
...
void build_edges() {
    ...
    int i, j, w, y, edge_edge_num = 0, index;
    fwd_two_step_edge_vec = (EdgeVec*) malloc(sizeof(EdgeVec) * useful_edge_num);
    __fwd_edge_edges = (Edge*) malloc(sizeof(Edge) * 40000000);
    ...
    for (u=0; u<node_num; ++u) {
        for (i=0; i<fwd_edges_vec[u].length; ++i) {
            ...
            fwd_two_step_edge_vec[index].from = __fwd_edge_edges + edge_edge_num;
            ...
            for (j=0; j<fwd_edges_vec[v].length; ++j) {
                ...
                __fwd_edge_edges[edge_edge_num].v = w;
                __fwd_edge_edges[edge_edge_num].x = y;
                edge_edge_num ++;
