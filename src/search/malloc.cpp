...
// #define INPUT_PATH    "resources/test_data.N111314.E200W.A19630345.txt"
...
#define MAX_BCK_1             1000000
#define MAX_BCK_2             5000000
#define MAX_BCK_3             10000000
...
void read_input_file() {
    input_buffer = (char*) malloc(MAX_EDGE * MAX_FOR_EACH_LINE);
...
void* reader_deser_##tid(void* args) { \
    ...
    int sorted_num = 0; \
    input_data_sorted_##tid = (int*) malloc(sizeof(int) * (EACH_EDGE_NUM << 1)); \
...
//     printf("where: %d\n", input_data_sorted_unique_num_##tid); fflush(stdout);
    sorted_vertex = (int**)malloc(sizeof(int*) * 2);
    sorted_vertex[0] = (int*)malloc(sizeof(int) * MAX_NODE);
    sorted_vertex[1] = (int*)malloc(sizeof(int) * MAX_NODE);
    int length = 0;
    // todo multi thread
...
#define def_hash_input_data(tid) \
    ...
    free(sorted_vertex[0]);
    free(sorted_vertex[1]);
    free(sorted_vertex);
...
#define def_edge_num_add(tid) \
    edge_num += input_data_num_##tid; 
...
// 改成先define 数据结构，再fwd，再bck，重用数组
#define def_malloc_raw_edges(nid) \
    raw_edges_chain_header_##nid = (int*) malloc(sizeof(int) * node_num); \
    memset(raw_edges_chain_header_##nid, 0, sizeof(int) * node_num); \
    raw_edges_chain_nxt_##nid = (int*) malloc(sizeof(int) * edge_num); \
    raw_edges_chain_v_##nid = (int*) malloc(sizeof(int) * edge_num); \
    raw_edges_chain_value_##nid = (int*) malloc(sizeof(int) * edge_num); \
    \
    raw_edges_header_##nid = (int*) malloc(sizeof(int) * node_num); \
    memset(raw_edges_header_##nid, 0, sizeof(int) * node_num); \
    raw_edges_v_fake_##nid = (int*) malloc(sizeof(int) * edge_num); \
    raw_edges_value_fake_##nid = (long long*)malloc(sizeof(long long) * edge_num); \
    \
    raw_edges_v_##nid = (int*) malloc(sizeof(int) * edge_num); \
    raw_edges_value_##nid = (long long*) malloc(sizeof(long long) * edge_num); \
    raw_edges_id_##nid = (int*) malloc(sizeof(int) * edge_num);


#define def_build_raw_edges(nid) \
int raw_edges_chain_num_##nid = 1; \
int* raw_edges_chain_header_##nid; \
int* raw_edges_chain_nxt_##nid; \
int* raw_edges_chain_v_##nid; \
int* raw_edges_chain_value_##nid; \
\
int raw_edges_num_##nid = 0; \
int* raw_edges_header_##nid; \
\
int* raw_edges_v_fake_##nid; \
long long* raw_edges_value_fake_##nid; \
\
int* raw_edges_v_##nid; \
long long* raw_edges_value_##nid; \
int* raw_edges_id_##nid; \
...
#define def_malloc_rev_edges(nid) \
    raw_rev_edges_chain_v_##nid = (int*) malloc(sizeof(int) * edge_num); \
    raw_rev_edges_chain_header_##nid = (int*) malloc(sizeof(int) * node_num); \
    raw_rev_edges_chain_nxt_##nid = (int*) malloc(sizeof(int) * edge_num); \
    raw_rev_edges_chain_value_##nid = (long long*) malloc(sizeof(long long) * edge_num);

#define def_build_rev_edges(nid) \
int raw_rev_edges_chain_num_##nid = 1; \
int* raw_rev_edges_chain_v_##nid; \
int* raw_rev_edges_chain_header_##nid; \
int* raw_rev_edges_chain_nxt_##nid; \
long long* raw_rev_edges_chain_value_##nid; \
...
#define def_print(X) printf(X); fflush(stdout);

#define def_free_raw_edges(nid) \
    free(raw_edges_chain_header_##nid);  \
    free(raw_edges_chain_nxt_##nid); \
    free(raw_edges_chain_v_##nid); \
    free(raw_edges_chain_value_##nid); \
    \
    free(raw_edges_header_##nid); \
    free(raw_edges_value_fake_##nid);  \
    \
    free(raw_edges_v_##nid); \
    free(raw_edges_value_##nid); \
    free(raw_edges_id_##nid); \
    \
    free(raw_rev_edges_chain_v_##nid); \
    free(raw_rev_edges_chain_header_##nid); \
    free(raw_rev_edges_chain_nxt_##nid); \
    free(raw_rev_edges_chain_value_##nid);
...
#define def_build_rev_edges_for(nid) \
    ...
    def_free_raw_edges(0)
    def_free_raw_edges(1)
    def_free_raw_edges(2)
    def_free_raw_edges(3)
    def_free_raw_edges(4)
    def_free_raw_edges(5)
}
...
#define def_malloc_search(tid) \
    fwd_starter_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_1_visit_##tid = (int*) malloc(sizeof(int) * node_num); \
    memset(bck_search_1_visit_##tid, 0, sizeof(int) * node_num); \
    bck_search_1_queue_edges_##tid = (int*) malloc(sizeof(int) * MAX_BCK_1); \
    bck_search_1_queue_values_##tid = (long long*) malloc(sizeof(long long) * MAX_BCK_1); \
    bck_search_1_queue_header_##tid = (int*) malloc(sizeof(int) * node_num); \
    \
    bck_search_2_visit_##tid = (int*) malloc(sizeof(int) * node_num); \
    memset(bck_search_2_visit_##tid, 0, sizeof(int) * node_num); \
    bck_search_2_queue_values_##tid = (long long*) malloc(sizeof(long long) * MAX_BCK_2); \
    bck_search_2_queue_header_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_2_queue_nxt_##tid = (int*) malloc(sizeof(int) * MAX_BCK_2); \
    \
    bck_search_2_sorted_visit_##tid = (int*) malloc(sizeof(int) * node_num); \
    memset(bck_search_2_sorted_visit_##tid, 0, sizeof(int) * node_num); \
    bck_search_2_sorted_id_from_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_2_sorted_id_to_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_2_sorted_id_##tid = (int*) malloc(sizeof(int) * MAX_BCK_2); \
    \
    bck_search_3_visit_##tid = (int*) malloc(sizeof(int) * node_num); \
    memset(bck_search_3_visit_##tid, 0, sizeof(int) * node_num); \
    bck_search_3_queue_values_##tid = (long long*) malloc(sizeof(long long) * MAX_BCK_3); \
    bck_search_3_queue_header_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_3_queue_nxt_##tid = (int*) malloc(sizeof(int) * MAX_BCK_3); \
    \
    bck_search_3_sorted_visit_##tid = (int*) malloc(sizeof(int) * node_num); \
    memset(bck_search_3_sorted_visit_##tid, 0, sizeof(int) * node_num); \
    bck_search_3_sorted_id_from_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_3_sorted_id_to_##tid = (int*) malloc(sizeof(int) * node_num); \
    bck_search_3_sorted_id_##tid = (int*) malloc(sizeof(int) * MAX_BCK_3); \
    \
    fwd_search_1_queue_edges_##tid = (int*) malloc(sizeof(int) * MAX_BCK_1); \
    fwd_search_1_queue_values_##tid = (long long*) malloc(sizeof(long long) * MAX_BCK_1); \
    \
    fwd_search_2_queue_values_##tid = (long long*) malloc(sizeof(long long) * MAX_BCK_2);

#define def_free_search(tid) \
    free(fwd_starter_##tid); \
    free(bck_search_1_visit_##tid); \
    free(bck_search_1_queue_edges_##tid); \
    free(bck_search_1_queue_values_##tid); \
    free(bck_search_1_queue_header_##tid); \
    \
    free(bck_search_2_visit_##tid); \
    free(bck_search_2_queue_values_##tid); \
    free(bck_search_2_queue_header_##tid); \
    free(bck_search_2_queue_nxt_##tid); \
    \
    free(bck_search_2_sorted_visit_##tid); \
    free(bck_search_2_sorted_id_from_##tid); \
    free(bck_search_2_sorted_id_to_##tid); \
    free(bck_search_2_sorted_id_##tid); \
    \
    free(bck_search_3_visit_##tid); \
    free(bck_search_3_queue_values_##tid); \
    free(bck_search_3_queue_header_##tid); \
    free(bck_search_3_queue_nxt_##tid); \
    \
    free(bck_search_3_sorted_visit_##tid); \
    free(bck_search_3_sorted_id_from_##tid); \
    free(bck_search_3_sorted_id_to_##tid); \
    free(bck_search_3_sorted_id_##tid); \
    \
    free(fwd_search_1_queue_edges_##tid); \
    free(fwd_search_1_queue_values_##tid); \
    \
    free(fwd_search_2_queue_values_##tid);

#define def_do_search(tid) \
int total_answer_num_##tid = 0; \
int answer_##tid[MAX_ANSW / 4][7]; \
int answer_from_##tid[5][MAX_NODE], answer_to_##tid[5][MAX_NODE]; \
int answer_num_##tid = 0, answer_bck_num_##tid = MAX_ANSW / 4; \
\
int* fwd_starter_##tid; \
...
int* bck_search_1_visit_##tid; \
int bck_search_1_mask_##tid = 123; \
\
int bck_search_1_queue_num_##tid = 0; \
int* bck_search_1_queue_edges_##tid; \
long long* bck_search_1_queue_values_##tid; \
int* bck_search_1_queue_header_##tid; \
\
...
\
int* bck_search_2_visit_##tid; \
int bck_search_2_mask_##tid = 1234; \
\
int bck_search_2_queue_num_##tid = 0; \
int bck_search_2_queue_edges_##tid[MAX_BCK_2][2]; \
long long* bck_search_2_queue_values_##tid; \
int* bck_search_2_queue_header_##tid; \
int* bck_search_2_queue_nxt_##tid; \
\
int* bck_search_2_sorted_visit_##tid; \
int* bck_search_2_sorted_id_from_##tid; \
int* bck_search_2_sorted_id_to_##tid; \
int* bck_search_2_sorted_id_##tid; \
int bck_search_2_sorted_id_num_##tid; \
\
...
\
int* bck_search_3_visit_##tid; \
int bck_search_3_mask_##tid = 12345; \
\
int bck_search_3_queue_num_##tid = 0; \
int bck_search_3_queue_edges_##tid[MAX_BCK_3][3]; \
long long* bck_search_3_queue_values_##tid; \
int* bck_search_3_queue_header_##tid; \
int* bck_search_3_queue_nxt_##tid; \
\
int* bck_search_3_sorted_visit_##tid; \
int* bck_search_3_sorted_id_from_##tid; \
int* bck_search_3_sorted_id_to_##tid; \
int* bck_search_3_sorted_id_##tid; \
int bck_search_3_sorted_id_num_##tid; \
\
...
int* fwd_search_1_queue_edges_##tid; \
long long* fwd_search_1_queue_values_##tid; \
\
...
long long* fwd_search_2_queue_values_##tid; \
\
...
void* serialize_##tid(void* args) { \
    s_buffer_##tid = (char* ) malloc(385000000); \
    int* prop = (int*) args; \
    if (prop[0] == -1) { \
...
void fire_ser() {
    serialize_buffer_length = (int*) malloc(sizeof(int) * node_num);
